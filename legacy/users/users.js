...
async function fetchAllUsers() {
  ...
  listContainer.innerHTML =
    '<div class="loading" id="loadingText">Loading users...</div>';
  ...
  try {
    ...
    console.error(error);
    listContainer.innerHTML =
      '<div class="loading">Error loading users. Check console.</div>';
  }
}
...
async function LoadUserProfile(basicUser) {
  ...
  try {
    ...
    document.getElementById("userName").textContent = `${
      basicUser.display_name || basicUser.username
    } (#${basicUser.id})`;
    ...
    if (userData.member_since || basicUser.created_at) {
      ...
      joinText = joinDate.toLocaleDateString() + " " +
    ...
    // Counts
    document.getElementById("userFriends").textContent =
      userData.friend_count ?? 0;
    document.getElementById("userPlaces").textContent = userData.place_count ??
      0;
